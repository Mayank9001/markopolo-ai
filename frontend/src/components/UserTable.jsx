...
const UserTable = React.memo(({ search }) => {
  ...
  const columns = useMemo(() => [
    {
      ...
      cell: ({ getValue }) => <div className="truncate w-60">{getValue()}</div>,
      enableSorting: true,
    },
    {
      accessorKey: "email",
      ...
      cell: ({ getValue }) => <div className="truncate w-72">{getValue()}</div>,
      enableSorting: true,
    },
    {
      accessorKey: "phone",
  ...
  // MayankVishwakarma
  // Github-https://github.com/Mayank9001
  const data = useMemo(() => (
    users.map((user) => ({
      ...user,
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      timeout = setTimeout(handleScroll, 400);
    };
